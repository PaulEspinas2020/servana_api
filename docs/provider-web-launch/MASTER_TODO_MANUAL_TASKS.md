...
| M-34 | **TAB 01's prescription is no longer available and the Master Command should record it.** Mandate 5 says "Run Verify in a GitHub-hosted job (fixed, known resources)". As of 2026-08-19 the owner has stopped topping up this repo's Actions credit, so every `ubuntu-latest` job — `release-gate.yml`, `fresh-db.yml` — is manual-only and cannot run. The gate has moved to `scripts/hooks/pre-push`. | An owner decision already taken; recorded here so the launch certification does not cite a gate that cannot run. | OPEN |
