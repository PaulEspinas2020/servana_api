...
| M-06 | ~~Fix or remove `fresh-db.yml`~~ **CLOSED 2026-08-20 by deletion.** Every workflow in every repository is gone; there is no red check left to train anyone to ignore. The gate itself survives as `npm run db:verify -- --live`, run by an operator — see `docs/FRESH_DB_GATE_LOCALLY.md`. | — | CLOSED |
...
## From TAB 10 — edge security headers (added 2026-08-18)

| # | Task | Why it is manual | Status |
|---|---|---|---|
| M-24 | **Run the CSP report-only for a full week with a report endpoint, then promote it to enforcing.** The guardrail forbids enforcing a policy that has not spent a week in report-only; Firebase auth and socket.io are the likeliest breakages. Needs a deploy and a reports sink. | Requires deploying and then observing production traffic across every provider flow. | OPEN |
| M-25 | **Remove `'unsafe-inline'` from `script-src` before the CSP is enforced.** It is carried from the meta policy, not endorsed. Removing it is a change to the build artefact, so it must not be bundled with the header change or a breakage becomes unattributable. | Requires the report-only week first, then an artefact change and a re-run of the flows. | OPEN — **must precede enforcement** |
| M-26 | **API-side headers and preflight, on the nginx that fronts `api.servana.com.ph`**: add HSTS and `X-Content-Type-Options: nosniff` (the API origin serves neither), and make an `OPTIONS` from a non-allowlisted origin answer a clean CORS rejection instead of a routing status (measured 404 today). | Production server configuration. Outside this programme's boundary — no production access. | OPEN |
| M-27 | **Verify `includeSubDomains` is safe for every Servana subdomain before ever adding `preload`.** HSTS preload is effectively irreversible. | An infrastructure audit across `api.`, `admin.` and any staging host. | OPEN |

## From TAB 11 — observability (added 2026-08-18)

| # | Task | Why it is manual | Status |
|---|---|---|---|
| M-28 | **Choose and provision an error-monitoring vendor**, then wire it to `servana-event-scrubber.ts` and configure source-map upload into the release gate (maps must NOT ship to browsers). | Needs an account, a DSN and a budget decision. Adding the SDK is additionally blocked by M-21 — any dependency change currently rewrites 30,000+ lines of lockfile. | OPEN — **blocks TAB 11 mandates 1, 5 and the alerting half of 4** |
| M-29 | **Configure the four alert signals and rehearse each one firing to a named human**: (a) any 404 or uncoded 401 on an `/api/v1` path, (b) session-expired redirects per signed-in session, (c) job-action failure rate by action, (d) earnings and payout read failures. Thresholds are specified in `TAB11_OBSERVABILITY.md`. | Needs the monitor (M-28) and an on-call owner. The command requires all four watched to fire in a rehearsal. | OPEN |
| M-30 | **Stand up the synthetic canary**: sign in as a dedicated provider account every fifteen minutes, load the jobs list, alert on failure. The command calls this "the single check that would have caught today's state on the day it began". | Needs the isolated provider account (M-08) and a scheduler. The account must not be matchable to live customer bookings. | OPEN |

## From TAB 12 — live certification (added 2026-08-18)

| # | Task | Why it is manual | Status |
|---|---|---|---|
| M-31 | **Make the production build reproducible before TAB 20's artefact check can pass.** A clean local build of `e57259d` — the exact commit production serves — produced `main.306c9c0f728753f3.js` while production serves `main.e26e7eec019dc8c4.js`. Same source, different bundle, because Netlify builds Node 20 with `--legacy-peer-deps`, this machine builds Node 24 with npm 11, and the lockfile is npm 6 format. | The fix is M-21, and proving it needs a Netlify build. Until then TAB 20 mandate 4 — "compare the served bundle hashes against a local build of the tagged commit" — **cannot pass for any commit**. | OPEN — **blocks TAB 20 mandate 4** |
| M-32 | **Any post-deploy artefact check must compare CONTENT, not status.** The SPA redirect answers `200` with `index.html` for any missing path, so a bundle-existence check that trusts a 200 passes for a bundle that is not there. | A rule for whoever writes the deploy smoke; recorded so the obvious check is not the one that ships. | OPEN |

## From the 2026-08-19 deploy failures (added 2026-08-19)

| # | Task | Why it is manual | Status |
|---|---|---|---|
| M-33 | **The deploy has failed five times running, all at "Typecheck (source and tests)" on the self-hosted runner.** Measured cause: `tsc --noEmit` peaks at **599 MB** and `tsc -p tsconfig.tests.json` at **710 MB**, on a host `deploy.yml` records as having **961 MB** — before the OS and the Actions agent. Same capacity failure as TAB 01's exit 134, relocated from the test suite into typecheck. **Recommended fix: drop the typecheck step from `deploy.yml`.** It is already covered by `npm run verify` in the pre-push hook, which since `fca1ed1` passes credential-free (verified: 4 suites, 32 tests, no key on disk). | Editing what runs immediately before a database migration and a PM2 restart is a production-deploy change, in a repository someone is actively pushing. Not a change to make unannounced. | OPEN — **production is healthy but running older code than `main`** |
| M-34 | **TAB 01's prescription is unavailable and the Master Command should record it.** Mandate 5 says "Run Verify in a GitHub-hosted job (fixed, known resources)". There is no such job: on 2026-08-20 the owner ruled out CI entirely and every workflow file was deleted, superseding the 2026-08-19 decision to merely stop topping up credit. The gate is `scripts/hooks/pre-push`. | An owner decision already taken; recorded so the launch certification does not cite a gate that cannot run. | CLOSED — recorded |
