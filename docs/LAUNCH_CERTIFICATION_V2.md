...
| G-03 | ~~**Production cannot identify itself.**~~ **CLOSED 2026-08-19.** The live `deploy.yml` had no stamping step — it existed only in the parked copy the PAT cannot push, so the endpoint was built, contracted, documented and fed by nothing. The stamp now happens in `npm run build`, which needs no workflow edit and also covers a manual build on the host — the case a deploy-time stamp never could | 2026-08-19 | **Closed.** Verify after the next deploy: `/api/v1/health` returns a commit |
