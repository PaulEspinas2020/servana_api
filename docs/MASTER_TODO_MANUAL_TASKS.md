...
| V10.6 | **The portal is not deploying at all, and `[skip ci]` does not fully explain it.** Two commits pushed deliberately without the marker (`cb923ed`, `7afca10`) produced no deploy over ~40 minutes; the live `runtime.<hash>.js` and `styles.<hash>.css` still differ from a local `build:prod`. Check the Netlify dashboard: auto-publish enabled, correct repo and production branch, and whether recent builds failed. A failed build leaves the previous deploy serving, so this looks like nothing happening | `PROD-ACCESS` | A push to `main` changes the live `runtime.<hash>.js` |
