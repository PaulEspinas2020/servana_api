...
import { contractDigest, CONTRACT_DIGEST_HEADER } from '../src/api/v1/contractDigest';
...
/**
 * The version a client pins against (TAB 03 mandate 4).
 *
 * `contractDigest()` is the sha256 of the OpenAPI document the RUNNING PROCESS
 * serves, and every `/api/v1` response already carries it as
 * `x-contract-sha256`. Stamping the same value here is what makes a stale pin
 * DETECTABLE rather than merely possible: a client compares the digest in its
 * pinned manifest against the header on any live response, and a mismatch means
 * its copy is old. Before this, the manifest carried no version of any kind, so
 * a client had no way to ask the question at all.
 *
 * Deliberately NOT a timestamp. This artifact is regenerated by
 * `convergence:docs:check` on every gate run, and a clock in it would make the
 * check fail on an unchanged contract. The digest changes when — and only when
 * — the contract does, which is exactly the property a version needs.
 */
  ...
  return `${JSON.stringify(
    {
      ...
      contractDigestHeader: CONTRACT_DIGEST_HEADER,
      $version:
        'contractSha256 is the sha256 of the OpenAPI document this API serves, and every ' +
        '/api/v1 response carries the same value in the ' + CONTRACT_DIGEST_HEADER + ' header. ' +
        'Compare the two to find out whether this pinned copy is stale. It is NOT a hash of ' +
        'this file.',
      $requestBody:
        'requiredBody / allowedBody / additionalBodyAllowed are DERIVED from the same SCHEMAS ' +
        'map the OpenAPI document is rendered from, so the two cannot disagree. All three are ' +
        'null when an operation declares no request body — null and [] mean different things, ' +
        'and an empty allow-list would read as "no field is permitted". ' +
        'bodyEnforcement says how the contract is enforced: no middleware validates a request ' +
        'body against these schemas, so each handler enforces its own preconditions. Read ' +
        'requiredBody as "the handler refuses without this", not as "a validator rejects this".',
      bodyEnforcement: 'handler',
      endpointCount: manifest.length,
      writeCount: writes.length,
      writesWithDeclaredBody: writes.filter((e) => e.requestSchema !== null).length,
